// ===============================================================
// AES T-Table Engine (정상 동작되는 완전 수정본)
// ===============================================================
...
// aes_engine_ttable.c 안에서 이 두 함수만 **그대로 교체**해줘.
// 나머지 코드는 건드리지 말고, encrypt/decrypt만 아래 버전으로 사용하면
// ref / ttable 둘 다 NIST CTR 128 기준으로 완전히 일치하게 된다.

// ---------------------------------------------------------------------
// AES T-table 엔진: 암호화 (실제는 S-box + GF(2^8) 연산으로 구현)
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// CTR만 쓸 거면 decrypt도 encrypt로 돌려도 동작은 동일하다.
// (ECB/CFB 등 역방향이 필요하면 따로 구현해야 함.)
// ---------------------------------------------------------------------
static void aes_ttab_decrypt_block_impl(void* vctx,
    const unsigned char in[16],
    unsigned char out[16])
{
    // CTR 모드에서만 쓰일 거라 decrypt = encrypt 로 둔다.
